...
Project GitHub Repository: github.com/kotack1/DukeForest-TackRiosMoyer
...
-   Figure \@ref(fig:MoonPhaseandTOD). Moon Phases and Time of Day
-   Figure \@ref(fig:MonthandTOD). Deer Observations by Month and Time of Day
-   Figure \@ref(fig:MonthandMoon). Deer Observations by Month and Moon Phase
-   Figure \@ref(fig:Heat1). Heatmap of Deer Observations by Moon Phase and Time of Day
-   Figure \@ref(fig:Heat2). Heatmap of Deer Observations by Time of Day and Camera Trap Location
...
: Table summary of deer_data structure
...
data_table %>% kbl() %>% kable_paper("hover", full_width = F) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), position = "center")
...
```{r VisualPlot, echo=FALSE, fig.cap='Deer Sightings by Hour over Three Months in Spring 2023', message=FALSE, warning=FALSE}
deer_data <- deer_data %>%
...
### Herd Size Scatter Plots {.unnumbered}
